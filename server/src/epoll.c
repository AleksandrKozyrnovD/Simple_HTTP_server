...
int epoll_socket_create(char *host, int port)
{
    ...
    if (listen(sfd, CONN_REQ_QUEUE) == -1)
    {
        perror("listen error");
        return -1;
    }
...
int epoll_loop(struct server *server)
{
    ...
    setnonblocking(server->sfd);

    event.data.fd = server->sfd;
    event.events = EPOLLIN;
    if (epoll_ctl(epoll_fd, EPOLL_CTL_ADD, server->sfd, &event) == -1)
    {
    ...
    while (server->running)
    {
        ...
        {
            if (events[i].data.fd == server->sfd)
            {
                client_socket = accept(server->sfd,(struct sockaddr *)&client_addr, &client_len);
                if (client_socket == -1)
                {
                    perror("accept");
                    continue;
