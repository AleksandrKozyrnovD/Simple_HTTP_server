...
void send_status(int clientfd, int status)
{
    if (clientfd == -1)
    {
        LOG_ERROR("send_status: bad clientfd %d\n", clientfd);
        close(clientfd);
        return;
    }
...
void send_file(char* path, int clientfd) {
    ...
    if (buff_resp == NULL)
    {
        LOG_ERROR("failed alloc resp buf: %s\n", strerror(errno));
        return;
    }
    ...
    while (1)
    {
        ...
        {
            byte_write = write(clientfd, buff_resp + total_write_temp,byte_read - total_write_temp);
            if (byte_write < 0)
            {
                if (errno == EAGAIN || errno == EWOULDBLOCK)
                {
                    LOG_WARN("write: %s; errno = %d\n", strerror(errno), errno);
                    usleep(10000);  // 10ms
                    continue;
                }
                LOG_ERROR("write: %s; errno = %d\n", strerror(errno), errno);
                break;
            }
...
void handle_http_request(int clientfd, http_request_t* req, char* static_dir)
{
    ...
    if (path == NULL)
    {
        LOG_ERROR("failder alloc path buff: %s\n", strerror(errno));
        return;
    }
    ...
    if(full_path == NULL)
    {
        ...
        LOG_ERROR("failed to allocate memory for full path: %s\n", strerror(errno));
        free(path);
        return;
    }
